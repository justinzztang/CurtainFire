...
(*tokens? idk they get converted*)

type pathing =
  | Stop (*no movement, setVelocity 0 0*)
  | Custom of number_expression * number_expression (*setVelocity theta speed*)
  | Custom_rectangular of
      number_expression * number_expression (*convert then Custom*)
  | Forwards of number_expression (*only set speed*)
  | Point of
      number_expression
      * number_expression
      * number_expression (*based on point, calculate angle and set speed*)
  | Orbit of
      number_expression
      * number_expression
      * number_expression
      * number_expression (*point, omega, radial speed*)
  | Gravitate of
      number_expression
      * number_expression
      * number_expression (*add a calculated acceleration to velocity*)
  | Drift of number_expression (*same as above, but only change speed*)
  | Steer of
      number_expression
      * number_expression (*angular delta and speed, tank controls*)
  | Angled of number_expression * number_expression (*abs angle and speed*)
  | Angled_rel of
      number_expression
      * number_expression (*rel angle and speed, turn then forward*)
  | Turn of number_expression (*update angle relatively*)

type spawning =
  | Angled (*modify*)
  | Pulse (*loop of Once -> Conditional Nothing | Elapsed t*)
  | Arc (*For of Modify_XY f(r), Modify_theta f(x)=x-angle/2+angle/n*i *)
  | Aimed (*Modify_theta f(x) = arctan player stuff *)
  | Spin (*loop of Modify_theta f(x) = v($LIFETIME) *)
